...
The MLB Stats API schedule endpoint is accepted only as a limited-use `FALLBACK` for Game schedule and Canonical game IDs under the forward-capture or deterministic-replay cutoff contract.

All other rows remain `UNPROVEN`.
